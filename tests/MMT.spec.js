...
test('test', async ({ page }) => {
  ...
  await page.getByText('Hyderabad, India').click();
  await page.getByText('9₹ 4,583').click();
  await page.locator('a').filter({ hasText: 'Search' }).click();
  await page.locator('.commonOverlay > span').click();
  await page.mouse.up();
  await page.locator('[id="bookbutton-RKEY\\:6e6c5eff-3416-40c2-ad41-913f91e269ba\\:22_0"]').click();
  ...
  await page1.getByPlaceholder('Last Name').press('Control+Shift+T');
  ...
  await page1.getByPlaceholder('Email').fill('s');
